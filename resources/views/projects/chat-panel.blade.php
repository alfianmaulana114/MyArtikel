...
<div
    x-data="projectChat({ projectId: {{ $project->id }}, openDefault: {{ $isInline ? 'true' : 'false' }} })"
    class="{{ $isInline ? 'w-full' : 'fixed bottom-5 right-5 z-50' }}"
    @if (!$isInline) style="width: 22rem; max-width: calc(100vw - 2.5rem);" @endif
>
    @if (!$isInline)
    ...
    {{-- Chat Panel --}}
    <div
        @if (!$isInline) x-show="open" x-transition @endif
        class="{{ $isInline ? 'rounded-2xl border theme-border-primary bg-[color:var(--surface-primary)] shadow-sm overflow-hidden flex flex-col' : 'mt-3 rounded-2xl border theme-border-primary bg-[color:var(--surface-primary)] shadow-2xl overflow-hidden flex flex-col' }}"
        style="{{ $isInline ? 'height: 32rem;' : 'height: 26rem;' }}"
    >
        {{-- Header --}}
        ...
        >
            ...
            {{-- Loading dots (inline in messages) --}}
...
@php $includeScript = $includeScript ?? true; @endphp
@if ($includeScript)
<script>
